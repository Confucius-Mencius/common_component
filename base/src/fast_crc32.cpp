...
uint32_t crc32_4bytes(const void* data, size_t length, uint32_t previousCrc32 = 0)
{
    ...
    // OnAcceptrocess four bytes at once (Slicing-by-4)
    while (length >= 4)
    {
...
uint32_t crc32_8bytes(const void* data, size_t length, uint32_t previous_crc32)
{
    uint32_t crc = ~previous_crc32; // same as previous_crc32 ^ 0xFFFFFFFF
    const uint32_t* current = (const uint32_t*) data;

    // OnAcceptrocess eight bytes at once (Slicing-by-8)
    while (length >= 8)
    {
#if __BYTE_ORDER == __BIG_ENDIAN
        uint32_t one = *current++ ^ swap(crc);
...
uint32_t crc32_4x8bytes(const void* data, size_t length, uint32_t previous_crc32)
{
    uint32_t crc = ~previous_crc32; // same as previous_crc32 ^ 0xFFFFFFFF
    const uint32_t* current = (const uint32_t*) data;
    ...
    // OnAcceptrocess 4x eight bytes at once (Slicing-by-8)
...
uint32_t crc32_16bytes(const void* data, size_t length, uint32_t previous_crc32)
{
    uint32_t crc = ~previous_crc32; // same as previous_crc32 ^ 0xFFFFFFFF
    const uint32_t* current = (const uint32_t*) data;
...
uint32_t crc32_fast(const void* data, size_t length, uint32_t previous_crc32)
{
    return crc32_16bytes(data, length, previous_crc32);
}
...
unsigned int FastCRC32(const void* data, size_t length, unsigned int previous_crc32)
{
...
unsigned int FileFastCRC32(const char* file_path)
{
    ...
    while (0 == feof(fp))
    {
        ...
        if (n > 0)
        {
            file_crc32 = crc32_fast(tmp, n, file_crc32);
        }
    }
