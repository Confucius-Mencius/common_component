...
/**
 * @brief 判断是否为非法的idx，取值范围[min, max)
 * @hideinitializer
 */
...
/**
 * @brief 检查能否在当前值上加上一个值
 * @param [in] cur_val 当前值
 * @param [in] delta 增加的值，为正数
 * @param [in] limit 数值的逻辑上限，该值可以取到（闭区间）
 * @param [in] max_val T类型所能表示的最大值，该值可以取到（闭区间）
 * @param [in] exceed 是否允许超出逻辑上限，如果为true，则最多加到max_val，结果会被截断
 * @retval true 可以操作
 * @retval false 不能操作，数值将溢出逻辑上限
 */
template<typename Type>
bool CanPlus(Type cur_val, Type delta, Type limit, Type max_val, bool exceed = false)
{
    if (delta <= 0)
    {
        return false;
    ...
    if (exceed)
    {
        return ((cur_val >= max_val) ? false : true);
    }
    ...
    if (limit > 0 && cur_val < 0) // 异号
    {
        if (cur_val + delta <= limit)
        {
            return true;
        }
    ...
    return (((limit - cur_val) < delta) ? false : true);
}
 ...
 */
template<typename Type>
Type Plus(Type cur_val, Type delta, Type limit, Type max_val, bool exceed = false)
{
    if (!CanPlus(cur_val, delta, limit, max_val, exceed))
    {
        return cur_val;
    }
    ...
    if (exceed)
    {
        if (max_val > 0 && cur_val < 0) // 异号
        {
            return cur_val + delta;
        }

        return ((delta < (max_val - cur_val)) ? (cur_val + delta) : max_val);
    }
...
/**
 * @brief 检查能否在当前值上减去一个值
 * @param [in] cur_val 当前值
 * @param [in] delta 减少的值，为正数
 * @param [in] limit 数值的逻辑下限，该值可以取到（闭区间）
 * @param [in] min_val T类型所能表示的最小值，该值可以取到（闭区间）
 * @param [in] exceed 是否允许超出逻辑下限，如果为true，则最多减到min_val，结果会被截断
 * @retval true 可以操作
 * @retval false 不能操作，数值将溢出逻辑下限
 */
template<typename Type>
bool CanMinus(Type cur_val, Type delta, Type limit, Type min_val, bool exceed = false)
{
    if (delta <= 0)
    {
        return false;
    ...
    if (exceed)
    {
        return ((cur_val <= min_val) ? false : true);
    }
    ...
    return (((cur_val - limit) < delta) ? false : true);
}
 ...
 * @param [in] delta 减少的值，为正数
 * @param [in] limit 数值的逻辑下限，该值可以取到（闭区间）
 * @param [in] max_val T类型所能表示的最下值，该值可以取到（闭区间）
 * @param [in] exceed 是否允许超出逻辑下限，如果为true，则最多减到min_val，结果会被截断
 * @return 减少后的值
 */
template<typename Type>
Type Minus(Type cur_val, Type delta, Type limit, Type min_val, bool exceed = false)
{
    if (!CanMinus(cur_val, delta, limit, min_val, exceed))
    {
        return cur_val;
    }
    ...
    if (exceed)
    {
        if (min_val < 0 && cur_val > 0) // 异号
        {
            return cur_val - delta;
        }

        return ((delta < (cur_val - min_val)) ? (cur_val - delta) : min_val);
    }
...
/**
 * @brief 两个i32类型的正数a、b相乘，结果放在ret中
 * @param [out] ret 乘积
 * @param [in] a 要求大于0
 * @param [in] b 要求大于0
 * @return 返回true表示计算成功，结果放在ret中了，返回false表示参数有误或者乘积会超出I32_MAX
 */
bool Multiply(int& ret, int a, int b);
 ...
 * @brief 求平方根的倒数
 */
inline float QuickReciprocalSQRT(float x)
{
    if (x < 0)
    {
        return -1;
