...
void ThreadSink::OnTask(const Task* task)
{
    ...
    switch (task_ctx->task_type)
    {
        case TASK_TYPE_NORMAL:
        {
            ...
            // TODO 临时实现
            MsgHead rsp_msg_head = task_ctx->msg_head;
            rsp_msg_head.msg_id = MSG_ID_NONE_HANDLER_FOUND;

            scheduler_.SendToTcpClient(&task_ctx->conn_guid, rsp_msg_head, NULL, 0);
        }
        break;
...
{
    if (0 == threads_ctx_->conf_mgr->GetWorkLocalLogicSo().length())
    {
        return 0;
    }

    const std::string local_logic_so_path = GetAbsolutePath(threads_ctx_->conf_mgr->GetWorkLocalLogicSo().c_str(),
...
int ThreadSink::LoadLogicGroup()
{
    ...
    const StrGroup logic_so_group = threads_ctx_->conf_mgr->GetWorkLogicSoGroup();
