...
void ThreadSink::SetRelatedThreadGroup(RelatedThreadGroups* related_thread_groups)
{
...
{
    if (0 == threads_ctx_->app_frame_conf_mgr->GetBurdenCommonLogicSo().length())
    {
        return 0;
    }
    ...
    GetAbsolutePath(common_logic_so_path, sizeof(common_logic_so_path),
                    threads_ctx_->app_frame_conf_mgr->GetBurdenCommonLogicSo().c_str(), threads_ctx_->cur_working_dir);
    LOG_TRACE("load local logic so " << common_logic_so_path << " begin");

    if (common_logic_loader_.Load(common_logic_so_path) != 0)
    {
        LOG_ERROR("failed to load local logic so, " << common_logic_loader_.GetLastErrMsg());
        return -1;
    }
    ...
    {
        LOG_ERROR("failed to get local logic interface, " << common_logic_loader_.GetLastErrMsg());
        return -1;
    }

    LOG_TRACE("load local logic so " << common_logic_so_path << " ...");
...
int ThreadSink::LoadLogicGroup()
{
    ...
    const StrGroup logic_so_group = threads_ctx_->app_frame_conf_mgr->GetBurdenLogicSoGroup();
