...
int MsgDispatcher::AttachMsgHandler(MsgId msg_id, base::MsgHandlerInterface* msg_handler)
{
...
void MsgDispatcher::DetachMsgHandler(MsgId msg_id)
{
    MsgHandlerMap::iterator it = msg_handler_map_.find(msg_id);
    if (it != msg_handler_map_.end())
    {
...
int MsgDispatcher::DispatchMsg(const ConnGuid* conn_guid, const MsgHead& msg_head, const void* msg_body,
                               size_t msg_body_len)
{
