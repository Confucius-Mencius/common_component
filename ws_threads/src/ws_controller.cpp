...
//                    strncpy(netmask, inet_ntoa(((struct sockaddr_in*)ifp->ifa_netmask)->sin_addr), sizeof(netmask));
    ...
    // wss
    if (threads_ctx_->conf_mgr->GetWSSecurityPort() > 0)
    {
...
int Controller::CreateWSContext(bool security)
{
    ...
    if (security)
    {
        info->port = threads_ctx_->conf_mgr->GetWSSecurityPort();
    }
    else
    {
...
//    };
    ...
    if (security)
    {
        strcpy(cert_file_path_, threads_ctx_->conf_mgr->GetWSCertificateChainFilePath().c_str());
        LOG_ALWAYS("cert file: " << cert_file_path_);

        strcpy(private_key_file_path_, threads_ctx_->conf_mgr->GetWSPrivateKeyFilePath().c_str());
        ...
        opts |= LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT; // app frame中的ssl init不要了
    ...
    info->max_http_header_data2 = LWS_RECOMMENDED_MIN_HEADER_SPACE; // The max amount of header payload that can be handled in an http request
    ...
    info->timeout_secs = threads_ctx_->conf_mgr->GetWSNetworkRoundtripTimeout(); // If nonzero, this member lets you set the timeout used in seconds. Otherwise a default timeout is used.
    info->keepalive_timeout = threads_ctx_->conf_mgr->GetHTTPKeepaliveTimeout(); // seconds to allow remote client to hold on to an idle HTTP/1.1 connection, 0 = 5s
    info->mounts = NULL;
    info->ws_ping_pong_interval = threads_ctx_->conf_mgr->GetWSPingPongInterval(); // 0 for none, else interval in seconds between sending PINGs on idle websocket connections.
