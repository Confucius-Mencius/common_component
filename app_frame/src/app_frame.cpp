...
namespace app_frame
{
AppFrame::AppFrame() : app_frame_ctx_(), conf_mgr_(), global_thread_loader_(), work_threads_loader_(),
    burden_threads_loader_(), tcp_threads_loader_(), http_threads_loader_(), udp_threads_loader_(),
    raw_tcp_threads_loader_()
{
    release_free_mem_date_ = 0;
    app_frame_threads_count_ = 0;
    global_threads_ = NULL;
    work_threads_ = NULL;
    burden_threads_ = NULL;
    tcp_threads_ = NULL;
    http_threads_ = NULL;
    udp_threads_ = NULL;
...
void AppFrame::Release()
{
    ...
    SAFE_RELEASE_MODULE(udp_threads_, udp_threads_loader_);
    SAFE_RELEASE_MODULE(http_threads_, http_threads_loader_);
    ...
    SAFE_RELEASE_MODULE(global_threads_, global_thread_loader_);
...
int AppFrame::Initialize(const void* ctx)
{
    ...
    // 不管有没有http服务，在app frame的初始化中调一次OpenSSLInit，保证只有一次调用。
    OpenSSLInit();
    ...
    LOG_INFO("default tcmalloc free mem release rate: " << MallocExtension::instance()->GetMemoryReleaseRate());
    MallocExtension::instance()->SetMemoryReleaseRate(5);
    LOG_INFO("our tcmalloc free mem release rate: " << MallocExtension::instance()->GetMemoryReleaseRate());
#endif
...
void AppFrame::Finalize()
{
    ...
    SAFE_FINALIZE(udp_threads_);
    SAFE_FINALIZE(http_threads_);
...
int AppFrame::Activate()
{
    ...
    if (SAFE_ACTIVATE_FAILED(http_threads_))
    {
        return -1;
    }

    if (SAFE_ACTIVATE_FAILED(udp_threads_))
    {
        return -1;
    }
...
void AppFrame::Freeze()
{
    ...
    SAFE_FREEZE(udp_threads_);
    SAFE_FREEZE(http_threads_);
...
bool AppFrame::CanExit() const
{
    ...
    if (tcp_threads_ != NULL)
    {
        if (tcp_threads_->GetListenThreadGroup() != NULL)
        {
            ...
            LOG_TRACE("tcp listen thread can exit: " << can_exit);
        }
...
int AppFrame::LoadAndCheckConf()
{
    ...
    /**
     * 1，通过检查是否配置有监听端口来确定服务是否存在（tcp、http、udp、raw tcp）
...
int AppFrame::LoadGlobalThread()
{
    ...
    char GLOBAL_THREAD_SO_PATH[MAX_PATH_LEN + 1] = "";
...
int AppFrame::LoadWorkThreads()
{
    ...
    char WORK_THREADS_SO_PATH[MAX_PATH_LEN + 1] = "";
...
int AppFrame::LoadBurdenThreads()
{
    ...
    char BURDEN_THREADS_SO_PATH[MAX_PATH_LEN + 1] = "";
...
int AppFrame::LoadTcpThreads()
{
    ...
    char TCP_THREADS_SO_PATH[MAX_PATH_LEN + 1] = "";
...
int AppFrame::LoadHttpThreads()
{
    ...
    char HTTP_THREADS_SO_PATH[MAX_PATH_LEN + 1] = "";
...
int AppFrame::LoadUdpThreads()
{
    ...
    char UDP_THREADS_SO_PATH[MAX_PATH_LEN + 1] = "";
...
void AppFrame::SetThreadsRelationship()
{
    ...
    if (tcp_threads_ != NULL)
    {
        ...
        tcp_threads_->SetRelatedThreadGroup(&tcp_related_thread_group);
    }
