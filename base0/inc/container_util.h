...
#define RELEASE_KV_CONTAINER(container)\
do {\
    auto tmp_container = container;\
    for (auto it: tmp_container)\
    {\
...
#define INITIALIZE_KV_CONTAINER(container, ctx)\
({\
    int ret = 0;\
    for (auto it: container)\
    {\
...
#define FINALIZE_KV_CONTAINER(container)\
do {\
    for (auto it: container)\
    {\
        it.second->Finalize();\
...
#define ACTIVATE_KV_CONTAINER(container)\
({\
    int ret = 0;\
    for (auto it: container)\
    {\
...
#define FREEZE_KV_CONTAINER(container)\
do {\
    for (auto it: container)\
    {\
        it.second->Freeze();\
