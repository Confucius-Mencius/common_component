...
static void launch_request(struct session* s)
{
    ...
    conn = evhttp_connection_base_bufferevent_new(base, 0, bev, s->host,
                                                  s->port);
    ...
    struct evkeyvalq* output_headers = evhttp_request_get_output_headers(
        new_req);
    ...
    if (s->kind == EVHTTP_REQ_POST)
    {
        evbuffer_add(evhttp_request_get_output_buffer(new_req), s->data,
                     s->data_size);
    }

    int suc = evhttp_make_request(conn, new_req, (evhttp_cmd_type) s->kind,
                                  "/java/sso/token");
    if (suc != 0)
    {
...
//    SSL_CTX_load_verify_locations(sctx, "certificate-authorities.pem", NULL); // 应该是客户端的校验

    SSL_CTX_set_verify(sctx, SSL_VERIFY_NONE, NULL); // 如果为SSL_VERIFY_PEER表示客户端会校验？
...
//    free(urlencoded_passcode);
    ...
    {
        host = "127.0.0.1";
    } /* horribly ugly hack to avoid ::1 for localhost */
#endif

    s1.kind = EVHTTP_REQ_GET;
    s1.host = host;
    s1.port = port;
...
static const char host[] = "account.moon.com";
