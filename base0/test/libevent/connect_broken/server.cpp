...
static void TimerCallback(evutil_socket_t fd, short event, void* arg)
{
    ...
    int n = bufferevent_write(bev, buf, 1048576);
    const int err = errno;
    printf("write\n");
    if (n < 0)
    {
...
void write_cb(struct bufferevent* bev, void* arg)
{
    ...
    struct evbuffer* evbuf = bufferevent_get_output(bev);
    size_t len = evbuffer_get_length(evbuf);
    printf("len before flush: %lu\n", len);

    int n = bufferevent_flush(bev, EV_WRITE, BEV_FLUSH);
    const int err = errno;
    printf("flush\n");
    if (n < 0)
    {
        printf("flush err: %d %s\n", err, strerror(err));
        return;
    }

//    struct evbuffer* evbuf = bufferevent_get_output(bev);
    len = evbuffer_get_length(evbuf);
    printf("len after flush: %lu\n", len);
}
...
void AcceptCallback(evutil_socket_t sockfd, short event_type, void* arg)
{
    ...
    printf("ACCEPT: fd = %u\n", fd);
//    evutil_make_socket_nonblocking(fd); // fd默认时阻塞的，使用evbuf的方式读写数据时不受影响，所以这里可以注释掉
