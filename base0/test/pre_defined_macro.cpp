/*
 * 显示各种C/C++编译器的预定义宏（C11标准、C++11标准、VC、BCB、Intel、GCC）
 */
#include "test_util.h"
...
/* Compilers */
const MACRO_T g_compilers[] = {{"[Compiler]", ""},
...
/* Operation system */
const MACRO_T g_platforms[] = {{"[Platform]", ""},
...
const MACRO_T g_borland[] = {{"[Borland C++]", ""},
#ifdef __BCOPT__
...
const MACRO_T g_intel_win[] = {{"[Intel C++: Windows]", ""},
...
const MACRO_T g_intel_linux[] = {{"[Intel C++: Linux]", ""},
...
static void Test001()
{
    ...
    print_MACRO_T(g_intel_linux,
                  sizeof(g_intel_linux) / sizeof(g_intel_linux[0]));
    print_MACRO_T(g_gcc, sizeof(g_gcc) / sizeof(g_gcc[0]));
    print_MACRO_T(g_others, sizeof(g_others) / sizeof(g_others[0]));
}

ADD_TEST(PreDefinedMacroTest, Test001);
