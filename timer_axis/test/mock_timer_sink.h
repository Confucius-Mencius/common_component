...
{
public:
    MOCK_METHOD4(OnTimer,
                 void(TimerID
                     timer_id,
                     const void* async_data, size_t
                     async_data_len, int
                     times));
};
...
{
public:
    void SetCtx(struct event_base* event_base, TimerAxisInterface* timer_axis, TimerSinkInterface* sink)
    {
...
public:
    ...
    void Delegate(struct event_base* event_base, TimerAxisInterface* timer_axis)
    {
        timer_sink_.SetCtx(event_base, timer_axis, this);
...
public:
    ...
    virtual void OnTimer(TimerID id, const void* async_data, size_t async_data_len, int times)
    {
        LOG_TRACE("on timer, times: " << times);
...
public:
    ...
    void Delegate(struct event_base* event_base, TimerAxisInterface* timer_axis, const struct timeval& interval)
    {
        timer_sink_.SetCtx(event_base, timer_axis, this, interval);
...
{
public:
    void SetCtx(struct event_base* event_base, TimerAxisInterface* timer_axis, TimerSinkInterface* other_sink,
                TimerID other_id)
    {
        event_base_ = event_base;
        timer_axis_ = timer_axis;
...
public:
    ...
    void Delegate(struct event_base* event_base, TimerAxisInterface* timer_axis, TimerSinkInterface* other_sink,
                  TimerID other_id)
    {
        timer_sink.SetCtx(event_base, timer_axis, other_sink, other_id);
...
public:
    ...
    void Delegate(struct event_base* event_base, TimerAxisInterface* timer_axis, TimerSinkInterface* other_sink,
                  TimerID other_id, const struct timeval& other_interval)
    {
        timer_sink_.SetCtx(event_base, timer_axis, other_sink, other_id, other_interval);
...
{
public:
    void SetCtx(struct event_base* event_base, TimerAxisInterface* timer_axis, TimerSinkInterface* sink, int ntimes)
    {
        event_base_ = event_base;
        timer_axis_ = timer_axis;
        sink_ = sink;
...
public:
    ...
    void Delegate(struct event_base* event_base, TimerAxisInterface* timer_axis, int ntimes)
    {
        timer_sink_.SetCtx(event_base, timer_axis, this, ntimes);
