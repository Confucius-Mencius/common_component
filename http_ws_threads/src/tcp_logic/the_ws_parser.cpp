...
int Parser::OnFrameHeader(websocket_parser* parser)
{
    ...
    if (wsp->http_ws_logic_ != nullptr)
    {
        wsp->http_ws_logic_->RecordPartMsg(wsp->conn_id_);
    }
...
int Parser::OnFrameEnd(websocket_parser* parser)
{
    ...
    if (wsp->fin_)
    {
        ...
        if (wsp->http_ws_logic_ != nullptr)
        {
            ...
            wsp->http_ws_logic_->OnWSMsg(conn_closed, wsp->conn_id_, opcode, wsp->payloads_.data(), wsp->payloads_.size());
        }
