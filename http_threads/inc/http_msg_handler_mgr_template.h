#ifndef HTTP_THREADS_INC_HTTP_MSG_HANDLER_MGR_TEMPLATE_H_
#define HTTP_THREADS_INC_HTTP_MSG_HANDLER_MGR_TEMPLATE_H_
...
int MsgHandlerMgrTemplate<MsgHandler>::Initialize(const void* ctx)
{
    ...
    for (typename MsgHandlerVec::iterator it = msg_handler_vec_.begin();
...
void MsgHandlerMgrTemplate<MsgHandler>::Finalize()
{
    ...
    {
        MsgHandler* msg_handler = *it;
        msg_dispatcher_->DetachMsgHandler(msg_handler->GetHttpReqPath());
...
#endif // HTTP_THREADS_INC_HTTP_MSG_HANDLER_MGR_TEMPLATE_H_
